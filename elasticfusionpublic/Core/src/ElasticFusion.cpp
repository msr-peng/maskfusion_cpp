...
void ElasticFusion::savePly(const float *class_ids_table, const std::vector<float> &class_color_lookup) {
    std::string filename = saveFilename;
    filename.append("_semantic.ply");
    ...
    for(unsigned int i = 0; i < globalModel.lastCount(); i++) {
        ...
        if(pos[3] > confidenceThreshold && class_ids_table[i] >= 0) {
            validCount++;
        }
    }
    ...
    for(unsigned int i = 0; i < globalModel.lastCount(); i++) {
        ...
        if(pos[3] > confidenceThreshold && class_ids_table[i] >= 0) {
            int class_id = class_ids_table[i];
	    int col = class_color_lookup[class_id];
            Eigen::Vector4f nor = mapData[(i * 3) + 2];
            ...
            unsigned char r = int(col) >> 16 & 0xFF;
            unsigned char g = int(col) >> 8 & 0xFF;
            unsigned char b = int(col) & 0xFF;
