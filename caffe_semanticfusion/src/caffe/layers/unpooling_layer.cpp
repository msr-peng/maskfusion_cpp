...
void UnPoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  PoolingParameter unpool_param = this->layer_param_.pooling_param();
      ...
      for (int c = 0; c < channels_; ++c) {
        for (int ph = 0; ph < height_; ++ph) {
          for (int pw = 0; pw < width_; ++pw) {
            ...
            if (use_bottom_mask) {
              if (bottom_data[index] > top_data[static_cast<int>(bottom_mask[index])]) {
                top_data[static_cast<int>(bottom_mask[index])] = bottom_data[index];
                if (use_top_mask) {
                  top_mask[static_cast<int>(bottom_mask[index])] = 1.0;
                }
              }
            } else {
      ...
      for (int c = 0; c < channels_; ++c) {
        for (int ph = 0; ph < height_; ++ph) {
          for (int pw = 0; pw < width_; ++pw) {
            ...
            if (use_bottom_mask) {
              if (bottom_data[index] == top_data[static_cast<int>(bottom_mask[index])]) {
                bottom_diff[index] = top_diff[static_cast<int>(bottom_mask[index])]; 
              }
            } else {
              if (bottom_data[index] == top_data[static_cast<int>(bottom_mask[index])]) {
