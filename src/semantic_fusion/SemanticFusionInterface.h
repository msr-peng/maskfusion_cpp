...
public:
  ...
  void UpdateProbabilities(const Eigen::Matrix4f &mask_pose, cv::Mat &scores_matrix, cv::Mat &class_ids_matrix, const std::unique_ptr<ElasticFusionInterface>& map);
...
public:
  const int num_classes_ = 81;
  const int prior_sample_size_ = 100;
  const int max_components_ = 3000000;
  const float colour_threshold_ = 0.0;
  
  float new_class_probabilities[3000000*81] = {};
  float new_class_probabilities_buffer[3000000*81] = {};
  float new_class_max[3*3000000] = {};
  float new_class_max_buffer[3*3000000] = {};
  float new_rendered_class_probabilities[81*640*480] = {};
};
