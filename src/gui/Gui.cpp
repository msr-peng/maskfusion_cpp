  ...
  , class_colour_lookup_(class_colour_lookup)
{
  ...
  pangolin::CreateWindowAndBind("MSR_MaskFusion", width_, height_, window_params);
  ...
  pangolin::Display("infer").SetAspect(640.0f/480.0f);
  ...
  pangolin::Display("multi").SetBounds(pangolin::Attach::Pix(0),1/4.0f,pangolin::Attach::Pix(180),1.0).SetLayout(pangolin::LayoutEqualHorizontal)
    .AddDisplay(pangolin::Display("infer"))
  ...
  save_.reset(new pangolin::Var<bool>("ui.Save Semantic Map", false, false));
...
void Gui::displayMaskRCNNInference(const std::string & id, pangolin::GlTexture& img) {
  glDisable(GL_DEPTH_TEST);
  pangolin::Display(id).Activate();
